...
// =============================================
async function createAuditLog(adminId, adminEmail, action, actionType, options = {}) {
    try {
...
router.post("/login", async (req, res) => {
    try {
        ...
        // Find admin user
        const admin = await AdminUser_1.default.findByEmail(email);
        if (!admin) {
        ...
        // Create audit log
        await createAuditLog(admin._id.toString(), admin.email, "Admin Login", "security", {
            severity: "low",
            ipAddress: req.ip,
...
router.post("/setup", async (req, res) => {
    try {
        ...
        const existingAdmins = await AdminUser_1.default.countDocuments();
        if (existingAdmins > 0) {
            res
                .status(403)
        ...
        const admin = new AdminUser_1.default({
            email,
            name,
            role: "superadmin",
            permissions: ["*"],
    ...
    try {
        ...
        if (search) {
            query.$or = [
                ...
                { lastLoginIp: new RegExp(search, "i") },
            ];
        }
        // Status filter
        if (status) {
...
router.get("/users/:userId", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        ...
        const recentOffers = await OfferLog_1.default.find({ user: userId })
            .sort({ createdAt: -1 })
            .limit(20)
            .lean();
        // Get user's withdrawals
        const withdrawals = await Withdrawal_1.default.find({ user: userId })
            .sort({ createdAt: -1 })
            .limit(10)
            .lean();
        // Get referral stats
        const referralStats = await ReferralEarning_1.default.aggregate([
...
// Ban user
router.post("/users/:userId/ban", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { userId } = req.params;
        ...
        // Create audit log
        await createAuditLog(req.admin.userId, req.admin.email, "Ban User", "user_management", {
            targetType: "user",
            targetId: userId,
            reason,
            severity: "high",
...
// Unban user
router.post("/users/:userId/unban", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { userId } = req.params;
        const user = await User_1.default.findById(userId);
        if (!user) {
        ...
        // Create audit log
        await createAuditLog(req.admin.userId, req.admin.email, "Unban User", "user_management", {
            targetType: "user",
            targetId: userId,
            severity: "medium",
...
// Warn user
router.post("/users/:userId/warn", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { userId } = req.params;
        ...
        // Create audit log
        await createAuditLog(req.admin.userId, req.admin.email, "Warn User", "user_management", {
            targetType: "user",
            targetId: userId,
            reason,
            severity: "medium",
...
// Add/Deduct points
router.post("/users/:userId/points", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { userId } = req.params;
        ...
        // Create audit log
        await createAuditLog(req.admin.userId, req.admin.email, `${type === "add" ? "Add" : "Deduct"} Points`, "user_management", {
            targetType: "user",
            targetId: userId,
            reason,
            severity: "medium",
...
// Set hold time
router.post("/users/:userId/hold-time", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { userId } = req.params;
        ...
        await user.save();
        // Create audit log
        await createAuditLog(req.admin.userId, req.admin.email, "Update Hold Time", "user_management", {
            targetType: "user",
            targetId: userId,
            severity: "low",
            changes: {
...
router.post("/users/:userId/notes", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { userId } = req.params;
        const { note } = req.body;
        if (!note) {
...
router.get("/referrals/overview", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        // Top referrers
        const topReferrers = await ReferralEarning_1.default.aggregate([
            {
                $group: {
                    _id: "$referrer",
                    totalEarnings: { $sum: "$amountCents" },
        ...
        const referrerUsers = await User_1.default.find({ _id: { $in: referrerIds } })
            .select("username email avatarUrl")
            .lean();
        const topReferrersWithData = topReferrers.map((ref) => {
            const user = referrerUsers.find((u) => u._id.toString() === ref._id.toString());
            return {
                ...ref,
                user,
            };
        ...
        // Total stats
        const totalStats = await ReferralEarning_1.default.aggregate([
            {
                $group: {
                    _id: null,
                    totalEarnings: { $sum: "$amountCents" },
        ...
        // Recent referrals
        const recentReferrals = await ReferralEarning_1.default.find({})
...
router.post("/users/:userId/referral-rate", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { userId } = req.params;
        ...
        await user.save();
        // Create audit log
        await createAuditLog(req.admin.userId, req.admin.email, "Update Referral Rate", "referral", {
            targetType: "user",
            targetId: userId,
            severity: "low",
            changes: {
...
router.get("/referrals/fraud-detection", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        // Find users with same IP who referred each other
        const suspiciousUsers = await User_1.default.aggregate([
            {
                $match: {
                    lastLoginIp: { $ne: null },
                },
        ...
        const inactiveReferrals = await User_1.default.find({
            referredBy: { $ne: null },
...
// =============================================
// Get all offers
router.get("/offers", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
...
// Toggle offer status
router.post("/offers/:offerId/toggle", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        ...
        await offer.save();
        // Create audit log
        await createAuditLog(req.admin.userId, req.admin.email, "Toggle Offer Status", "offer", {
            targetType: "offer",
            targetId: offerId,
            severity: "low",
            changes: {
...
// Set offer hold time
router.post("/offers/:offerId/hold-time", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { offerId } = req.params;
        const { days } = req.body;
        const offer = await Offer_1.default.findById(offerId);
        ...
        await offer.save();
        // Create audit log
        await createAuditLog(req.admin.userId, req.admin.email, "Update Offer Hold Time", "offer", {
            targetType: "offer",
            targetId: offerId,
            severity: "low",
            changes: {
...
// Get offer logs
router.get("/offer-logs", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        ...
        const [logs, total] = await Promise.all([
            OfferLog_1.default.find(query)
                .populate("user", "username email avatarUrl")
                .sort({ createdAt: -1 })
                .skip(skip)
                .limit(Number(limit))
                .lean(),
            OfferLog_1.default.countDocuments(query),
        ]);
        res.json({
            data: logs,
            total,
...
router.post("/offer-logs/:logId/review", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { logId } = req.params;
        const { status, reason } = req.body; // status: 'approved' or 'rejected'
        const log = await OfferLog_1.default.findById(logId).populate("user");
        if (!log) {
        ...
        if (status === "approved") {
            ...
            if (user) {
                ...
                const eventType = offerContext.includes("survey")
                    ? "survey_completion"
                    : "offer_completion";
                (0, activityProgression_1.applyActivityEvent)(user, eventType, {
                ...
                if (log.provider === "premium" && mongoose_1.default.Types.ObjectId.isValid(log.offerId)) {
                    const today = new Date();
                    today.setHours(0, 0, 0, 0);
                    const premiumOffer = await PremiumOffer_1.default.findById(log.offerId);
                    if (premiumOffer) {
        ...
        // Create audit log
        await createAuditLog(req.admin.userId, req.admin.email, `${status === "approved" ? "Approve" : "Reject"} Offer`, "offer", {
            targetType: "offer",
            targetId: logId,
            reason,
            severity: "medium",
...
// =============================================
// Get all withdrawals
router.get("/withdrawals", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
...
router.post("/withdrawals/:withdrawalId/approve", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { withdrawalId } = req.params;
        const { giftCardCode, approvalNotes } = req.body;
        const withdrawal = await Withdrawal_1.default.findById(withdrawalId).populate("user");
        if (!withdrawal) {
        ...
        try {
            if (user.email) {
                await emailService_1.default.sendPayoutSuccessful({
        ...
        // Create audit log
        await createAuditLog(req.admin.userId, req.admin.email, "Approve Withdrawal", "payout", {
...
router.post("/withdrawals/:withdrawalId/reject", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { withdrawalId } = req.params;
        ...
        await withdrawal.save();
        // Send notification to user
        const notifUser = withdrawal.user;
        ...
        try {
            if (notifUser.email) {
                await emailService_1.default.sendPayoutRejected({
        ...
        // Create audit log
        await createAuditLog(req.admin.userId, req.admin.email, "Reject Withdrawal", "payout", {
            targetType: "withdrawal",
            targetId: withdrawalId,
            reason: rejectionReason,
            severity: "medium",
...
router.post("/withdrawals/:withdrawalId/complete", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { withdrawalId } = req.params;
        const withdrawal = await Withdrawal_1.default.findById(withdrawalId).populate("user");
        if (!withdrawal) {
        ...
        // Create audit log
        await createAuditLog(req.admin.userId, req.admin.email, "Complete Withdrawal", "payout", {
            targetType: "withdrawal",
            targetId: withdrawalId,
            severity: "low",
            metadata: {
...
router.get("/withdrawals/:withdrawalId/user-activity", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { withdrawalId } = req.params;
        ...
        const recentOffers = await OfferLog_1.default.find({ user: withdrawal.user })
            .sort({ createdAt: -1 })
            .limit(20)
            .lean();
        const recentWithdrawals = await Withdrawal_1.default.find({ user: withdrawal.user })
            .sort({ createdAt: -1 })
            .limit(10)
            .lean();
        res.json({
...
router.get("/users/:userId/activity", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { userId } = req.params;
        const user = await User_1.default.findById(userId);
        if (!user) {
        ...
        const recentOffers = await OfferLog_1.default.find({ user: userId })
            ...
            .limit(20)
            .lean();
        const recentWithdrawals = await Withdrawal_1.default.find({ user: userId })
            .sort({ createdAt: -1 })
            .limit(10)
            .lean();
        res.json({
...
// =============================================
// Create promo code
router.post("/promo-codes", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { code, amount, limit, expiresAt, description, promoType, maxUsesPerUser, } = req.body;
        ...
        const promoCode = await PromoCode_1.default.create({
        ...
        // Create audit log
        await createAuditLog(req.admin.userId, req.admin.email, "Create Promo Code", "promo", {
            targetType: "promo",
...
// List promo codes
router.get("/promo-codes", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
...
router.post("/promo-codes/:codeId/toggle", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { codeId } = req.params;
        const promoCode = await PromoCode_1.default.findById(codeId);
        if (!promoCode) {
        ...
        // Create audit log
        await createAuditLog(req.admin.userId, req.admin.email, "Toggle Promo Code", "promo", {
            targetType: "promo",
            targetId: codeId,
            severity: "low",
            changes: {
...
// Delete promo code
router.delete("/promo-codes/:codeId", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { codeId } = req.params;
        const promoCode = await PromoCode_1.default.findByIdAndDelete(codeId);
        if (!promoCode) {
            res.status(404).json({ error: "Promo code not found" });
            return;
        }
        // Create audit log
        await createAuditLog(req.admin.userId, req.admin.email, "Delete Promo Code", "promo", {
            targetType: "promo",
            targetId: codeId,
            severity: "medium",
...
router.post("/users/:userId/vpn-flag", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { userId } = req.params;
        ...
        // Create audit log
        await createAuditLog(req.admin.userId, req.admin.email, isVpn ? "Flag VPN User" : "Remove VPN Flag", "security", {
            targetType: "user",
            targetId: userId,
            reason,
            severity: "high",
...
router.get("/security/suspicious", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        // Users with high payouts
        const highPayoutUsers = await Withdrawal_1.default.aggregate([
            {
                $match: {
        ...
        const highPayoutWithUsers = highPayoutUsers.map((payout) => {
            const user = users.find((u) => u._id.toString() === payout._id.toString());
            return { ...payout, user };
        });
        // Users with many offers in short time
        const suspiciousOffers = await OfferLog_1.default.aggregate([
            {
                $match: {
        ...
        // VPN users
        const vpnUsers = await User_1.default.find({ isVpnUser: true })
            .select("username email lastLoginIp vpnDetectedAt vpnDetectionReason")
            .limit(50)
            .lean();
        // Users with duplicate IP
        const duplicateIpGrouping = await User_1.default.aggregate([
            {
                $match: {
                    lastLoginIp: { $ne: null },
                    isBanned: { $ne: true }
...
router.post("/security/review-activity", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { activityId, action, notes } = req.body;
        const user = await User_1.default.findById(activityId);
        if (!user) {
            res.status(404).json({ error: "User not found" });
            return;
        }
        if (action === "ban") {
            ...
            user.bannedAt = new Date();
            await user.save();
            await createAuditLog(req.admin.userId, req.admin.email, "Ban User (Security Review)", "security", {
                targetType: "user",
            ...
            await createAuditLog(req.admin.userId, req.admin.email, "Approve User (Security Review)", "security", {
                targetType: "user",
                targetId: user._id.toString(),
                reason: notes,
                ipAddress: req.ip
...
// =============================================
// Get all tickets
router.get("/support/tickets", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        ...
        const [tickets, total] = await Promise.all([
            SupportTicket_1.default.find(query)
                .populate("user", "username email avatarUrl")
                .sort({ createdAt: -1 })
                .skip(skip)
                .limit(Number(limit))
                .lean(),
            SupportTicket_1.default.countDocuments(query),
        ]);
...
// Get single ticket
router.get("/support/tickets/:ticketId", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
...
// Reply to ticket
router.post("/support/tickets/:ticketId/reply", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { ticketId } = req.params;
        ...
        const ticket = await SupportTicket_1.default.findById(ticketId).populate("user");
        if (!ticket) {
            res.status(404).json({ error: "Ticket not found" });
            return;
        }
        ...
        // Create audit log
        await createAuditLog(req.admin.userId, req.admin.email, "Reply to Support Ticket", "support", {
            targetType: "user",
            targetId: ticketId,
            severity: "low",
...
router.post("/support/tickets/:ticketId/status", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { ticketId } = req.params;
...
// Assign ticket
router.post("/support/tickets/:ticketId/assign", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { ticketId } = req.params;
...
router.post("/notifications/send", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        ...
        if (users && Array.isArray(users) && users.length > 0) {
            targetUsers = await User_1.default.find({ _id: { $in: users } })
                .select("_id")
                .lean();
        }
        else {
            targetUsers = await User_1.default.find({}).select("_id").lean();
        }
        ...
        // Create audit log
        await createAuditLog(req.admin.userId, req.admin.email, "Send Notification", "notification", {
            severity: "low",
...
router.get("/stats/dashboard", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const now = new Date();
        ...
        // User stats
        const [totalUsers, newUsersToday, newUsers7Days, newUsers30Days, activeUsers,] = await Promise.all([
            ...
            User_1.default.countDocuments({ lastLoginAt: { $gte: last7Days } }),
        ]);
        // Earnings stats
        const earningsStats = await OfferLog_1.default.aggregate([
            {
        ...
        // Payout stats
        const payoutStats = await Withdrawal_1.default.aggregate([
            {
        ...
        // Pending withdrawals
        const pendingWithdrawals = await Withdrawal_1.default.countDocuments({
        ...
        // Top offers
        const topOffers = await OfferLog_1.default.aggregate([
            { $match: { status: "approved" } },
            {
                $group: {
                    _id: "$offerId",
        ...
        // Top users
        const topUsers = await OfferLog_1.default.aggregate([
            { $match: { status: "approved" } },
            {
                $group: {
                    _id: "$user",
        ...
        const topUserData = await User_1.default.find({ _id: { $in: topUserIds } })
            .select("username email avatarUrl")
            .lean();
        const topUsersWithData = topUsers.map((user) => {
            const userData = topUserData.find((u) => u._id.toString() === user._id.toString());
...
router.get("/stats/charts", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        ...
        const registrations = await User_1.default.aggregate([
        ...
        // Earnings over time
        const earnings = await OfferLog_1.default.aggregate([
        ...
        // Payouts over time
        const payouts = await Withdrawal_1.default.aggregate([
            { $match: { createdAt: { $gte: startDate } } },
            {
                $group: {
                    _id: { $dateToString: { format: "%Y-%m-%d", date: "$createdAt" } },
...
// =============================================
// Get audit logs
router.get("/audit-logs", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
...
// =============================================
// System health check
router.get("/system/health", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
...
router.post("/system/clear-cache", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        // Implement cache clearing logic here
        // For example, if using Redis: await redis.flushall()
        await createAuditLog(req.admin.userId, req.admin.email, "Clear Cache", "system", {
            severity: "low",
            ipAddress: req.ip,
            userAgent: req.headers["user-agent"],
        });
...
router.post("/system/backup", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        // Implement backup logic here
        // This would typically involve creating a database dump
        await createAuditLog(req.admin.userId, req.admin.email, "Create Backup", "system", {
            severity: "medium",
            ipAddress: req.ip,
            userAgent: req.headers["user-agent"],
        });
...
router.post("/admin-users", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        ...
        const existing = await AdminUser_1.default.findByEmail(email);
        if (existing) {
        ...
        await admin.setPassword(password);
        await admin.save();
        await createAuditLog(req.admin.userId, req.admin.email, "Create Admin User", "system", {
            severity: "high",
            metadata: { email, role },
            ipAddress: req.ip,
            userAgent: req.headers["user-agent"],
...
// List admin users
router.get("/admin-users", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const admins = await AdminUser_1.default.find({})
...
router.post("/admin-users/:adminId/deactivate", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        if (req.admin.role !== "superadmin") {
            res
                .status(403)
        ...
        const admin = await AdminUser_1.default.findById(adminId);
        if (!admin) {
        ...
        await admin.save();
        await createAuditLog(req.admin.userId, req.admin.email, "Deactivate Admin User", "system", {
            severity: "high",
            targetId: adminId,
...
// =============================================
// Get system settings
router.get("/system/settings", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const settings = await SystemSettings_1.default.getSettings();
        await createAuditLog(req.admin.userId, req.admin.email, "View System Settings", "system", {
            severity: "low",
            ipAddress: req.ip,
            userAgent: req.headers["user-agent"],
        });
...
router.post("/system/settings", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        if (req.admin.role !== "superadmin") {
            res
                .status(403)
        ...
        if (updates.activityLevelThresholds !== undefined) {
            ...
            if (!Number.isFinite(beginnerMax) ||
                !Number.isFinite(amateurMax) ||
                !Number.isFinite(advancedMax) ||
                !Number.isFinite(proMax)) {
                res.status(400).json({
                    error: "activityLevelThresholds requires numeric beginnerMax/amateurMax/advancedMax/proMax",
                });
                return;
            }
            if (beginnerMax < 0 ||
                ...
                proMax <= advancedMax) {
                res.status(400).json({
                    error: "Level thresholds must satisfy: 0 <= beginnerMax < amateurMax < advancedMax < proMax",
                });
                return;
            }
        ...
        const oldValues = {
            ...
            activityScoreConfig: settings.activityScoreConfig,
            activityLevelThresholds: settings.activityLevelThresholds,
        };
        // Update settings
        Object.assign(settings, updates);
        await settings.save();
        await createAuditLog(req.admin.userId, req.admin.email, "Update System Settings", "system", {
            severity: "high",
            changes: {
                before: oldValues,
                after: {
...
router.post("/system/theme", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const settings = await SystemSettings_1.default.getSettings();
...
// Upload logo
router.post("/system/logo", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        if (req.admin.role !== "superadmin") {
            res
                .status(403)
        ...
        settings.logo = logo;
        await settings.save();
        await createAuditLog(req.admin.userId, req.admin.email, "Upload Logo", "system", {
            severity: "low",
            ipAddress: req.ip,
            userAgent: req.headers["user-agent"],
        });
...
router.get("/export/:type", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { type } = req.params;
        let data = [];
        let headers = [];
        if (type === "users") {
            data = await User_1.default.find({}).select("-passwordHash").lean();
            headers = [
                "username",
                "email",
                "balanceCents",
            ...
            data = await Withdrawal_1.default.find({})
                .populate("user", "username email")
                .lean();
            headers = ["user", "method", "amountCents", "status", "createdAt"];
        }
        else if (type === "offers") {
            data = await OfferLog_1.default.find({})
                .populate("user", "username email")
                .lean();
            headers = [
                "user",
        ...
        await createAuditLog(req.admin.userId, req.admin.email, `Export ${type} CSV`, "system", {
            severity: "medium",
...
router.get("/premium-offers", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { search, status, type, platform, surface, page = 1, limit = 20, sortBy = "createdAt", sortOrder = "desc", } = req.query;
        const query = {};
        // Search filter
        ...
        const [offers, total] = await Promise.all([
            PremiumOffer_1.default.find(query)
                .sort(sortOptions)
                .skip(skip)
                .limit(limitNum)
                .lean(),
            PremiumOffer_1.default.countDocuments(query),
        ]);
        // Get stats
        const [activeCount, totalPayout, totalCompletions] = await Promise.all([
            PremiumOffer_1.default.countDocuments({ status: "active" }),
            PremiumOffer_1.default.aggregate([
...
router.get("/premium-offers/:id", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const offer = await PremiumOffer_1.default.findById(id).lean();
        if (!offer) {
...
router.post("/premium-offers", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { title, description, imageUrl, trackingUrl, type, rewardCents, platform, status, requirements, country, priority, provider, expiresAt, showOnWelcomePage, showOnEarnPage, completionCap, } = req.body;
        ...
        const offer = new PremiumOffer_1.default({
            title,
            ...
            showOnWelcomePage: typeof showOnWelcomePage === "boolean" ? showOnWelcomePage : true,
        ...
        await offer.save();
        await createAuditLog(req.admin.userId, req.admin.email, "Create Premium Offer", "offers", {
...
router.put("/premium-offers/:id", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        if (!mongoose_1.default.Types.ObjectId.isValid(id)) {
            res.status(400).json({ error: "Invalid offer ID" });
        ...
        const offer = await PremiumOffer_1.default.findById(id);
        if (!offer) {
            res.status(404).json({ error: "Premium offer not found" });
            return;
        }
        ...
        if (trackingUrl !== undefined)
            offer.trackingUrl = trackingUrl;
        if (type !== undefined)
            offer.type = type;
        if (rewardCents !== undefined)
            offer.rewardCents = parseInt(rewardCents);
        ...
        if (priority !== undefined)
            offer.priority = parseInt(priority);
        if (provider !== undefined)
            offer.provider = provider;
        if (expiresAt !== undefined)
            offer.expiresAt = expiresAt || null;
        ...
        await offer.save();
        await createAuditLog(req.admin.userId, req.admin.email, "Update Premium Offer", "offers", {
            targetType: "PremiumOffer",
            targetId: id,
...
router.delete("/premium-offers/:id", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        if (!mongoose_1.default.Types.ObjectId.isValid(id)) {
            res.status(400).json({ error: "Invalid offer ID" });
        ...
        await PremiumOffer_1.default.findByIdAndDelete(id);
        await createAuditLog(req.admin.userId, req.admin.email, "Delete Premium Offer", "offers", {
            targetType: "PremiumOffer",
            targetId: id,
...
router.post("/premium-offers/:id/toggle", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        if (!mongoose_1.default.Types.ObjectId.isValid(id)) {
            res.status(400).json({ error: "Invalid offer ID" });
        ...
        offer.updatedBy = req.admin.email;
        await offer.save();
        await createAuditLog(req.admin.userId, req.admin.email, `Toggle Premium Offer ${offer.status === "active" ? "Active" : "Inactive"}`, "offers", {
            targetType: "PremiumOffer",
            targetId: id,
...
router.get("/recent-activity", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { limit = 20 } = req.query;
        const limitNum = parseInt(limit) || 20;
        // Get recent withdrawals (payouts/cashouts)
        const recentWithdrawals = await Withdrawal_1.default.find({ status: "Completed" })
            .populate("user", "username avatarUrl")
            .sort({ completedAt: -1 })
            .limit(limitNum)
            .lean();
        // Get recent offer completions (earnings)
        const recentEarnings = await OfferLog_1.default.find({ status: "approved" })
            .populate("user", "username avatarUrl")
            .sort({ createdAt: -1 })
            .limit(limitNum)
            .lean();
        // Get recent referral earnings
        const recentReferralEarnings = await ReferralEarning_1.default.find()
        ...
        recentReferralEarnings.forEach((r) => {
            activities.push({
                type: "referral",
                user: r.referrer,
...
const OfferWallSchema = new mongoose_1.default.Schema({
...
router.get("/offerwalls", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
...
router.get("/offerwalls/:id", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
...
router.post("/offerwalls", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { name, displayName, type, description, logoUrl, apiKey, apiSecret, callbackUrl, isActive, priority, commission, settings, } = req.body;
        if (!name || !displayName) {
        ...
        // Create audit log
        await createAuditLog(req.admin.userId.toString(), req.admin.email, `Created offerwall: ${displayName}`, "offerwall_management", {
...
router.put("/offerwalls/:id", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        allowedFields.forEach((field) => {
            if (updates[field] !== undefined) {
                offerwall[field] = updates[field];
            }
        });
        await offerwall.save();
        // Create audit log
        await createAuditLog(req.admin.userId.toString(), req.admin.email, `Updated offerwall: ${offerwall.displayName}`, "offerwall_management", {
            targetType: "offerwall",
            targetId: offerwall._id.toString(),
            severity: "low",
        });
...
router.delete("/offerwalls/:id", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const offerwall = await OfferWall.findById(id);
        if (!offerwall) {
        ...
        // Create audit log
        await createAuditLog(req.admin.userId.toString(), req.admin.email, `Deleted offerwall: ${displayName}`, "offerwall_management", {
            targetType: "offerwall",
            targetId: id,
            severity: "high",
        });
...
router.post("/offerwalls/:id/toggle", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const offerwall = await OfferWall.findById(id);
        if (!offerwall) {
        ...
        await offerwall.save();
        // Create audit log
        await createAuditLog(req.admin.userId.toString(), req.admin.email, `${offerwall.isActive ? "Activated" : "Deactivated"} offerwall: ${offerwall.displayName}`, "offerwall_management", {
            targetType: "offerwall",
            targetId: offerwall._id.toString(),
            severity: "medium",
        });
...
router.get("/offerwalls-stats", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const [totalOfferwalls, activeOfferwalls, offerwallsByType,] = await Promise.all([
        ...
        res.json({
            ...
            byType: offerwallsByType.reduce((acc, item) => {
...
router.get("/tournaments", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
...
router.post("/tournaments", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
...
router.put("/tournaments/:id", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const { name, imageUrl, status, prizePool, description, startDate, endDate, priority } = req.body;
        ...
        if (startDate !== undefined)
            tournament.startDate = startDate ? new Date(startDate) : null;
        if (endDate !== undefined)
            tournament.endDate = endDate ? new Date(endDate) : null;
        if (priority !== undefined)
...
router.delete("/tournaments/:id", requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const tournament = await Tournament_1.default.findByIdAndDelete(id);
        if (!tournament) {
            res.status(404).json({ error: "Tournament not found" });
            return;
        }
