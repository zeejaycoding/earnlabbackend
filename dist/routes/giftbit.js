...
const GiftbitTransaction_1 = __importDefault(require("../models/GiftbitTransaction"));
const router = (0, express_1.Router)();
/**
 * GET /api/v1/giftbit/brands
 * Get available gift card brands from Giftbit
 */
router.get('/giftbit/brands', async (_req, res, next) => {
    try {
 ...
 */
router.get('/giftbit/brands/:brandCode', async (req, res, next) => {
    try {
 ...
 */
router.post('/giftbit/payout/create', requireAuth_1.default, (0, express_validator_1.body)('brandCode').isString().notEmpty(), (0, express_validator_1.body)('brandName').isString().notEmpty(), (0, express_validator_1.body)('amountCents').isInt({ min: 100 }), (0, express_validator_1.body)('recipientEmail').isEmail(), (0, express_validator_1.body)('recipientName').optional().isString(), (0, express_validator_1.body)('giftMessage').optional().isString(), async (req, res, next) => {
    try {
        ...
        const transaction = await GiftbitTransaction_1.default.create({
            user: user._id,
            type: 'Payout',
            brandCode: brandCode,
            brandName: brandName,
 ...
 */
router.get('/giftbit/transactions', requireAuth_1.default, (0, express_validator_1.query)('limit').optional().isInt({ min: 1, max: 100 }), (0, express_validator_1.query)('offset').optional().isInt({ min: 0 }), async (req, res, next) => {
    try {
        const user = req.user;
        const limit = parseInt(req.query.limit) || 20;
        const offset = parseInt(req.query.offset) || 0;
        const transactions = await GiftbitTransaction_1.default.find({ user: user._id })
            .sort({ createdAt: -1 })
            .limit(limit)
            .skip(offset)
            .lean()
            .exec();
        const total = await GiftbitTransaction_1.default.countDocuments({ user: user._id });
        return res.json({
            success: true,
 ...
 */
router.get('/giftbit/transactions/:transactionId', requireAuth_1.default, async (req, res, next) => {
    try {
        const user = req.user;
        const { transactionId } = req.params;
        const transaction = await GiftbitTransaction_1.default.findOne({
            _id: transactionId,
            user: user._id,
        }).lean().exec();
        if (!transaction) {
 ...
 */
router.post('/giftbit/transactions/:transactionId/resend', requireAuth_1.default, async (req, res, next) => {
    try {
        const user = req.user;
        const { transactionId } = req.params;
        const transaction = await GiftbitTransaction_1.default.findOne({
 ...
 */
router.get('/giftbit/account/balance', requireAuth_1.default, async (req, res, next) => {
    try {
        const user = req.user;
...
//# sourceMappingURL=giftbit.js.map