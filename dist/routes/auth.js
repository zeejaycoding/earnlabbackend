...
function generateAffiliateCode() {
    ...
    return `REF${(0, uuid_1.v4)().replace(/-/g, "").slice(0, 8).toUpperCase()}`;
}
/**
 ...
 */
function handleValidationResult(req, res) {
 ...
 * }
 */
router.post("/register", (0, express_validator_1.body)("username").isString().isLength({ min: 3, max: 30 }).trim(), (0, express_validator_1.body)("email").isEmail().normalizeEmail(), (0, express_validator_1.body)("password").isString().isLength({ min: 8 }), (0, express_validator_1.body)("affiliateCode").optional().isString(), (0, express_validator_1.body)("agreedToTerms")
    .isBoolean()
    .custom((v) => v === true), async (req, res, next) => {
    try {
        ...
        // eslint-disable-next-line no-console
        console.log("[auth:register] existing user?", !!existing, existing
            ? {
                id: existing._id,
        ...
        newUser.lastLoginDevice = userAgent.slice(0, 200);
        ...
        if (clientIp && clientIp !== "unknown" && clientIp !== "::1" && clientIp !== "127.0.0.1") {
            const duplicateCount = await User_1.default.countDocuments({
                lastLoginIp: clientIp,
                _id: { $ne: newUser._id },
            }).exec();
            if (duplicateCount > 0) {
                newUser.adminNotes.push({
        ...
        if (newUser.referredBy) {
            const referrer = await User_1.default.findById(newUser.referredBy).exec();
            if (referrer) {
                const settings = await SystemSettings_1.default.getSettings().catch(() => null);
                (0, activityProgression_1.applyActivityEvent)(referrer, "successful_referral", {
 ...
 * body: { email, password }
 */
router.post("/login", (0, express_validator_1.body)("email").isEmail().normalizeEmail(), (0, express_validator_1.body)("password").isString().isLength({ min: 1 }), async (req, res, next) => {
    try {
        const validationErr = handleValidationResult(req, res);
        if (validationErr)
            return;
        ...
        user.lastLoginDevice = userAgent.slice(0, 200);
        ...
        const settings = await SystemSettings_1.default.getSettings().catch(() => null);
        (0, activityProgression_1.applyDailyLoginIfEligible)(user, settings?.activityScoreConfig);
        await user.save();
...
router.post("/clerk-sync", async (req, res, next) => {
    try {
        const { email, name: reqName, avatarUrl: reqAvatarUrl, } = req.body;
        ...
        if (process.env.NODE_ENV !== "production") {
            // eslint-disable-next-line no-console
            console.debug("[auth:clerk-sync] CLERK_SECRET_KEY present?", !!clerkKey);
        }
        if (!clerkKey) {
            return res
                .status(500)
        ...
        if (!user) {
            ...
            const base = (c.full_name || c.first_name || c.username || email.split("@")[0])
                .toString()
            ...
            let attempt = 0;
            // ensure uniqueness
            while (await User_1.default.findOne({ username: candidate }).exec()) {
                attempt += 1;
        ...
        const settings = await SystemSettings_1.default.getSettings().catch(() => null);
        const appliedDailyLogin = (0, activityProgression_1.applyDailyLoginIfEligible)(user, settings?.activityScoreConfig);
 ...
 */
router.post("/clerk-refresh", async (req, res, next) => {
    try {
        const { email, name: reqName, avatarUrl: reqAvatarUrl, } = req.body;
        if (!email || typeof email !== "string") {
            return res.status(400).json({ message: "email is required" });
    ...
    try {
        ...
        if (!user) {
            ...
            if (!user.passwordHash) {
                ...
                const salt = await bcryptjs_1.default.genSalt(10);
                user.passwordHash = await bcryptjs_1.default.hash(rndPwd, salt);
                changed = true;
            }
            if (changed) {
                await user.save();
        ...
        user.lastLoginIp = googleClientIp;
        user.lastLoginDevice = googleUserAgent.slice(0, 200);
        user.loginHistory.push({
            ip: googleClientIp,
